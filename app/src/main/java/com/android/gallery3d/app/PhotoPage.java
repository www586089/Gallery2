...
import static com.android.gallery3d.util.UsageStatistics.ACTION_SHARE;
import static com.android.gallery3d.util.UsageStatistics.COMPONENT_GALLERY;
...
public abstract class PhotoPage extends ActivityState implements
    ...
    private final PanoramaSupportCallback mUpdatePanoramaMenuItemsCallback = new PanoramaSupportCallback() {
        ...
        public void panoramaInfoAvailable(MediaObject mediaObject, boolean isPanorama,
            ...
            if (mediaObject == mCurrentPhoto) {
                mHandler.obtainMessage(MSG_UPDATE_PANORAMA_UI, isPanorama360 ? 1 : 0, 0, mediaObject).sendToTarget();
            }
        }
    };
        ...
        if (mSetPathString != null) {
            ...
            pda.setDataListener(new PhotoDataAdapter.DataListener() {
                ...
                public void onLoadingStarted() {
                    android.util.Log.d(TAG, "onLoadingStarted");
                }
            });
        } else {
    ...
    public void onBottomControlClicked(int control) {
        switch(control) {
            case R.id.photopage_bottom_control_edit:
                ...
                return;
            case R.id.photopage_bottom_control_panorama:
                mActivity.getPanoramaViewHelper().showPanorama(mCurrentPhoto.getContentUri());
                return;
            case R.id.photopage_bottom_control_tiny_planet:
                launchTinyPlanet();
                return;
    ...
    private void updateUIForCurrentPhoto() {
        ...
        // more clear
        if ((mCurrentPhoto.getSupportedOperations() & MediaObject.SUPPORT_ACTION) != 0 && !mPhotoView.getFilmMode()) {
            mPhotoView.setWantPictureCenterCallbacks(true);
        }
        ...
        if ((mSecureAlbum == null) && (mCurrentPhoto.getSupportedOperations() & MediaItem.SUPPORT_SHARE) != 0) {
    ...
    public void onFilmModeChanged(boolean enabled) {
        ...
        if (enabled) {
            mHandler.removeMessages(MSG_HIDE_BARS);
            UsageStatistics.onContentViewChanged(COMPONENT_GALLERY, "FilmstripPage");
        } else {
            refreshHidingMessage();
            if (mAppBridge == null || mCurrentIndex > 0) {
                UsageStatistics.onContentViewChanged(COMPONENT_GALLERY, "SinglePhotoPage");
            } else {
    ...
    public boolean onShareTargetSelected(ShareActionProvider source, Intent intent) {
        ...
        final long value = timestampMillis > 0 ? System.currentTimeMillis() - timestampMillis : -1;
        UsageStatistics.onEvent(COMPONENT_GALLERY, ACTION_SHARE, mediaType, value);
        return false;
    }
