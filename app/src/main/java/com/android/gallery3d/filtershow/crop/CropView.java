...
public class CropView extends View {
    ...
    private void setup(Context context) {
        ...
        dashPathEffect = new DashPathEffect(new float[]{mDashOnLength, mDashOnLength + mDashOffLength}, 0);
    }
    ...
    public void initialize(Bitmap image, RectF newCropBounds, RectF newPhotoBounds, int rotation) {
        ...
        if (mCropObj != null) {
            ...
            if (crop != newCropBounds || containing != newPhotoBounds || mRotation != rotation) {
                mRotation = rotation;
    ...
    public void onDraw(Canvas canvas) {
        ...
        mImageBounds.set(0, 0, mBitmap.getWidth(), mBitmap.getHeight());//= new RectF(0, 0, mBitmap.getWidth(), mBitmap.getHeight());
        mScreenBounds.set(0, 0, canvas.getWidth(), canvas.getHeight());// = new RectF(0, 0, canvas.getWidth(), canvas.getHeight());
        ...
        if (mDisplayMatrix == null || mDisplayMatrixInverse == null) {
            ...
            if (!CropDrawingUtils.setImageToScreenMatrix(mDisplayMatrix, mImageBounds, mScreenBounds, mRotation)) {
        ...
        if (mDisplayMatrix.mapRect(mScreenImageBounds)) {
            ...
            mShadowBounds.set(mShadowBounds.left - margin, mShadowBounds.top - margin, mShadowBounds.right + margin, mShadowBounds.bottom + margin);
        ...
        if (mDisplayMatrix.mapRect(mScreenCropBounds)) {
            ...
            normalPaint.setColor(mOverlayShadowColor);
            normalPaint.setStyle(Paint.Style.FILL);
            CropDrawingUtils.drawShadows(canvas, normalPaint, mScreenCropBounds, mScreenImageBounds);
            ...
            if (!mDoSpot) {
                ...
                wpPaint.setPathEffect(dashPathEffect);
                normalPaint.setColor(mOverlayWPShadowColor);
                CropDrawingUtils.drawWallpaperSelectionFrame(canvas, mScreenCropBounds, mSpotX, mSpotY, wpPaint, normalPaint);
            }
            CropDrawingUtils.drawIndicators(canvas, mCropIndicator, mIndicatorSize, mScreenCropBounds,
                    mCropObj.isFixedAspect(), decode(mCropObj.getSelectState(), mRotation));
        }
