...
/*__inline__*/ int interpLocal(unsigned char *src, int p, int *off, float dr, float dg, float db) {
...
void JNIFUNCF(ImageFilterFx, nativeApplyFilter, jobject bitmap, jint width, jint height,
    ...
    for (i = start; i < end; i += STEP) {
        ...
        float dg = fg - lut_g;
        float db = fb - lut_b;
        rgb[RED] = clamp(interpLocal(lutrgb, p, off, dr, dg, db));
        rgb[GREEN] = clamp(interpLocal(lutrgb, p + 1, off, dr, dg, db));
        rgb[BLUE] = clamp(interpLocal(lutrgb, p + 2, off, dr, dg, db));
    }
